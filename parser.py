...
def normalize_product(sku, nama_produk):
    ...
    s = sku.upper() if sku else ""

    if any(x in s for x in ["100GR", "100 GR"]):
        return "Black Garlic 100gr"
    if any(x in s for x in ["220GR", "220 GR"]):
        return "Black Garlic 220gr"
    if any(x in s for x in ["500GR", "500 GR"]):
        return "Black Garlic 500gr"
    if "DRINK" in s and any(x in s for x in ["PEACH", "PCH"]):
        return "Drink BG Peach"
    if "DRINK" in s:
        return "Drink BG Original"
    if any(x in s for x in ["FLORA", "MF-", "MULTIFLORA"]):
        return "Madu Multi Flora 260gr"
    if any(x in s for x in ["KURMA", "MK-", "BUNGA"]):
        return "Madu Bunga Kurma 260gr"

    n = nama_produk.upper() if nama_produk else ""

    if "PEACH" in n and "DRINK" in n:
        return "Drink BG Peach"
    if "DRINK" in n:
        return "Drink BG Original"
    if "100" in n and ("GR" in n or "GRAM" in n):
        return "Black Garlic 100gr"
    if "220" in n and ("GR" in n or "GRAM" in n):
        return "Black Garlic 220gr"
    if "500" in n and ("GR" in n or "GRAM" in n):
        return "Black Garlic 500gr"
    if any(x in n for x in ["FLORA", "MULTI FLORA"]):
        return "Madu Multi Flora 260gr"
    if any(x in n for x in ["KURMA", "BUNGA KURMA"]):
        return "Madu Bunga Kurma 260gr"
...
def detect(text):
    ...
    if "JET.CO.ID" in t or re.search(r"\bJX\d{10}\b", text):
        ...
        for s in ["EZ", "NDD", "ECO", "REG"]:
            if re.search(r"\b" + s + r"\b", text[:300].upper()):
                sv = s
                break
    ...
    if (
        re.search(r"No\.\s*Resi:\s*0046\d+|No\.\s*Resi:\s*00296", text)
        or re.search(r"\b0046\d{8,10}\b", text)
        or re.search(r"\b00296\d{7,9}\b", text)
    ):
        pl = "TikTok Shop" if "TIKTOK" in t else "Tokopedia/TikTok"
    ...
    if re.search(r"\bAAJ\w+\b", text) or "ANTERAJA" in t:
        return "Shopee", "Anteraja", "REG"

    if re.search(r"\bLEX\w+\b", text) or "LAZADA" in t:
        return "Lazada", "LEX", "REG"
...
def get_resi(text):
    patterns = [
        r"\b(JX\d{10})\b",
        r"\b(GTL\d{8,12})\b",
        r"No\.\s*Resi:\s*(0046\d+)",
        r"No\.\s*Resi:\s*(00296\d+)",
        r"\b(0046\d{8,10})\b",
        r"\b(00296\d{7,9})\b",
        r"\b(SPXID\d{10,15})\b",
        r"No\.\s*Resi:\s*(CM\d+)",
...
def get_alamat_tiktok(text):
    ...
    for line in lines:
        ...
        if re.search(r"Penerima\s*:|Receiver\s+", line):
            capture = True
            continue

        if capture:
            if re.search(
                r"Weight\s*:|Ship\s*:|Order\s*Id|Estimated|Shipping Date|Sender\s|TT Order|In transit",
                line,
                re.I,
            ):
                break
            ...
            if (
                l
                and not re.search(r"^\(?\+?62|^Pengirim|^Sender|DKI JAKARTA", l)
                and len(l) > 4
            ):
                addr.append(l)
...
def get_alamat_shopee(text):
    ...
    for i, line in enumerate(lines):
        ...
        if re.search(
            r"KAB\.|KOTA JAKARTA|JAWA|SUMATERA|KALIMANTAN|SULAWESI|BALI|BANTEN|NTB|RIAU|PAPUA|ACEH|KOTA\s+\w+",
            l.upper(),
        ):
            addr = []
            ...
            for j in range(max(0, i - 3), min(i + 3, len(lines))):
                ...
                if lj and len(lj) > 4 and not re.search(
                    r"Penerima|Pengirim|HSD|6281|COD|Berat|Batas|No\.|Reguler|CASHLESS|SPXID|BDO|Resi:|SKU|Variasi",
                    lj,
                    re.I,
                ):
                    addr.append(lj)
    ...
    if m:
        ...
        lns = [
            l
            for l in lns
            if not re.search(r"^\d{10,}|^Pengirim|^HSD|CASHLESS|COD|Resi:|SKU", l, re.I)
        ]
...
def parse_tiktok_products(text):
    ...
    main_re = re.compile(
        r"^(.*?)\s+(Default|220\s*GR|\b1\b|[A-Z0-9]{2,10})\s+([A-Z][A-Z0-9]*-[A-Z0-9\-]+)\s+(\d{1,2})\s*$"
    )
    ...
    while i < len(raw):
        ...
        if mm:
            ...
            while j < len(raw):
                if main_re.match(raw[j]):
                    break

                sf = sku_suf.search(raw[j])

                if sf:
                    seller_sku += sf.group(1)
                    j += 1
                    continue
...
def parse_shopee_products(text):
    ...
    main_re = re.compile(
        r"^(\d+)\s+.+?(BG-[A-Z0-9\-]+)\s+(220 GR|ORIGINAL \d+\s*BOTOL|DEFAULT|[\w\s]{2,25}?)\s+(\d{1,2})\s*$"
    )
